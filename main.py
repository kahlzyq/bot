...
class FollowRequestView(View):
    ...
    async def send_request(self, interaction: discord.Interaction, button: Button):
        ...
        staff_embed.add_field(name="ETA", value="10 min start — 20 min delivery", inline=False)
...
async def add_account(interaction: discord.Interaction, account: str):
    ...
    await interaction.response.send_message(f"✅ Added account to stock. Current stock: {len(stock)}", ephemeral=True)

@app_commands.command(name="gen", description="Generate an account from stock (DMs you).")
async def gen_account(interaction: discord.Interaction):
    ...
    try:
        ...
        await interaction.response.send_message("Failed to DM you!", ephemeral=True)
...
async def view_stock(interaction: discord.Interaction):
    ...
    if not stock:
        return await interaction.response.send_message("Stock is empty.", ephemeral=True)
    embed = discord.Embed(title="Current Stock", description="\n".join(stock), color=discord.Color.green())
    await interaction.response.send_message(embed=embed, ephemeral=True)

bot.tree.add_command(add_account)
bot.tree.add_command(gen_account)
bot.tree.add_command(view_stock)

# -------------------------
# Embed command with title, text, image
# -------------------------
@app_commands.command(name="embed", description="Send a custom embed as the bot.")
@app_commands.describe(
    title="Title of the embed",
    text="Main text (use \\n for line breaks)",
    image_url="Optional image URL"
)
async def embed_command(interaction: discord.Interaction, title: str, text: str, image_url: Optional[str] = None):
...
async def on_message(message: discord.Message):
    if message.author.bot:
        return
    if message.channel and getattr(message.channel, "id", None) == HELP_CHANNEL_ID:
        try:
            await message.delete()
        except Exception:
            pass
        try:
            await message.author.send(f"⚠️ Please ask for help in <#{HELP_CHANNEL_ID}>")
        except Exception:
            pass
